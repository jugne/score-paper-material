function [] = convertXmlToSimulation_segmentRootFilteringMax(folder, type_names)
...
for run =1:10
%     cd(folder)
    log_files = dir(['current/h5n1_reject_30DaysDiff_08_16/run_' num2str(run) '/output/segment_root_filter_max/h5n1_combined.rootFilter.log']);
    for i = 1 : length(log_files)
        t = importdata(['current/h5n1_reject_30DaysDiff_08_16/run_' num2str(run) '/output/segment_root_filter_max/' log_files(i).name]);
...
%                     end           
            ...
            system('mkdir simulation/posterior_rates/segment_root_filter_max');
            system(sprintf('mkdir simulation/posterior_rates/segment_root_filter_max/run_%s' , num2str(run)));
            g = fopen(['simulation/posterior_rates/segment_root_filter_max/run_' num2str(run) '/' strrep(log_files(i).name, '.log', '.sim.xml')], 'w');
            while ~feof(f)
                line = fgets(f);
