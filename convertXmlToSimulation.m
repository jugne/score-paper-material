function [] = convertXmlToSimulation(folder, type_names)
...
for run =1:10
%     cd(folder)
    log_files = dir(['current/h5n1_reject_30DaysDiff_08_16/run_' num2str(run) '/combined/h5n1_combined.log']);
    for i = 1 : length(log_files)
        t = importdata(['current/h5n1_reject_30DaysDiff_08_16/run_' num2str(run) '/combined/' log_files(i).name]);
...
%         if rea_ind~=-cell(0,0)
            ...
            f = fopen(['current/h5n1_reject_30DaysDiff_08_16/run_' num2str(run) '/xmls_coupled/' strrep(log_files(i).name, '_combined.log', '_rep0.xml')]);
...
%                     end           
            ...
            system(sprintf('mkdir -p simulation/posterior_rates/no_filter/run_%s' , num2str(run)));
            g = fopen(['simulation/posterior_rates/no_filter/run_' num2str(run) '/' strrep(log_files(i).name, '.log', '.sim.xml')], 'w');
            while ~feof(f)
                line = fgets(f);
